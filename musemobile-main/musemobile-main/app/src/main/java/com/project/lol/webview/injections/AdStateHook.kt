    ...
    const val CONTENT = """
        (function(){
            ...
            function isAdTrack(t){
                if (!t) return false;
                try {
                    var uri = t.uri || t.canonical_uri || t.content_uri || '';
                    if (typeof uri === 'string' && (uri.indexOf('spotify:ad:') === 0 || uri.indexOf('spotify:advertisement') === 0)) return true;
                } catch(e){}
                try {
                    var nm = t.name || t.title || (t.metadata && (t.metadata.title || t.metadata.name)) || '';
                    if (typeof nm === 'string' && nm.toLowerCase() === 'advertisement') return true;
                } catch(e){}
                var m = t.metadata || {};
                var cands = [
