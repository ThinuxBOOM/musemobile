...
/**
 * Web-player port of adblockify's five-layer model (rxri/spicetify-extensions/adblock).
 *
 * Desktop adblockify cannot run as-is on Android native (no webpack/CEF, Kotlin/ART + JNI
 * core instead), but MuseMobile is a WebView wrapper around open.spotify.com - which IS a
 * JS/webpack client like desktop. So the *kind* of attack ports, with web-player primitives:
 *
 *  Layer 1 - Product state lie: patch JSON payloads containing ads/catalogue/product/type
 *            (+ mobile-relevant on-demand/shuffle/player-license) on every fetch read.
 *  Layer 2 - Managers kill-switch: best-effort webpack theft to find Esperanto Settings/
 *            Slots/Testing clients and disable managers; no-op if web player build
 *            renamed them (ProGuard/chunk rename). Fetch-emptying below is the fallback.
 *  Layer 3 - Slots never commit: empty JSON for ad-only endpoints; poisoned settings
 *            re-applied on a 5s timer because Spotify rewrites them after init.
 *  Layer 4 - CSS: hide leftover upgrade/leaderboard/HPTO/home-ad chrome. Does NOT stop audio.
 *  Layer 5 - Remote config: force enableEsperantoMigration=true (so layers 2-3 stay on the
 *            live path), enableInAppMessaging=false, hideUpgradeCTA=true.
 *
 *  Plus MuseMobile-specific steady state: auto-skip ad tracks (spotify:ad: / "Advertisement")
 *  via actSkipForward()/skip button, since a pure mute would leave a 30s silent hole.
 *
 *  Safety (from research):
 *  - Never 404 the music CDN (gew4-spclient/scdn audio). Only empty ad-only hosts.
 *  - Never mutate live collections; allocate new Responses/lists.
 *  - Compose with FetchOverride (inner) + AdStateHook (middle): this wrapper installs
 *    OUTERMOST so AdStateHook still sees unpatched truth for ID harvest, while the page
 *    receives the patched lie.
 */
object Adblockify {
    const val CONTENT = """
        (function(){
