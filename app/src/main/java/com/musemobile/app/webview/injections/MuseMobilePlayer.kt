...
/*
 * CREDIT: MuseMobile - Custom Player.
 *
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣀⡀⠀⠀⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣾⠙⠻⢶⣄⡀⠀⠀⠀⢀⣤⠶⠛⠛⡇⠀⠀⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢹⣇⠀⠀⣙⣿⣦⣤⣴⣿⣁⠀⠀⣸⠇⠀⠀⠀
...
object MuseMobilePlayer {
    const val CONTENT = """
            window.initMuseMobilePlayer=function(){
                if(document.getElementById('musemobilePlayerControls')) return;
                ...
                pl.id='musemobilePlayerControls';
                pl.innerHTML=''
                    +'<div class="spl-top">'
                ...
                if(!document.getElementById('spl-vol-css')){
                    ...
                    sst.textContent='#musemobilePlayerControls .spl-vol-wrap{display:flex;align-items:center;gap:2px;margin-right:2px}#musemobilePlayerControls .spl-vol-btn{flex-shrink:0}#musemobilePlayerControls .spl-vol-bar{position:relative;width:70px;height:38px;display:flex;align-items:center;cursor:pointer;flex-shrink:0;margin:0 2px}#musemobilePlayerControls .spl-vol-track{position:absolute;left:0;right:0;top:50%;transform:translateY(-50%);height:4px;border-radius:2px;background:rgba(255,255,255,.14)}#musemobilePlayerControls .spl-vol-fill{position:absolute;left:0;top:50%;transform:translateY(-50%);height:4px;border-radius:2px;background:var(--spl-accent,#1db954);width:0%}#musemobilePlayerControls .spl-vol-handle{position:absolute;top:50%;left:0%;width:12px;height:12px;transform:translate(-50%,-50%);border-radius:50%;background:#fff;opacity:0;transition:opacity .15s;box-shadow:0 1px 4px rgba(0,0,0,.5);pointer-events:none}#musemobilePlayerControls .spl-vol-bar:hover .spl-vol-handle,#musemobilePlayerControls .spl-vol-bar:active .spl-vol-handle{opacity:1}#musemobilePlayerControls.spl-empty{opacity:0!important;pointer-events:none!important;transform:translateY(24px)!important}';
                    var t=document.head||document.documentElement;if(t)t.appendChild(sst);
                }

                if(!document.getElementById('spl-vol-css')){
                    var sst=document.createElement('style');sst.id='spl-vol-css';
                    sst.textContent='#musemobilePlayerControls .spl-vol-wrap{display:flex;align-items:center;gap:2px;margin-right:2px}#musemobilePlayerControls .spl-vol-btn{flex-shrink:0}#musemobilePlayerControls .spl-vol-bar{position:relative;width:70px;height:38px;display:flex;align-items:center;cursor:pointer;flex-shrink:0;margin:0 2px}#musemobilePlayerControls .spl-vol-track{position:absolute;left:0;right:0;top:50%;transform:translateY(-50%);height:4px;border-radius:2px;background:rgba(255,255,255,.14)}#musemobilePlayerControls .spl-vol-fill{position:absolute;left:0;top:50%;transform:translateY(-50%);height:4px;border-radius:2px;background:#1db954;width:0%}#musemobilePlayerControls .spl-vol-handle{position:absolute;top:50%;left:0%;width:12px;height:12px;transform:translate(-50%,-50%);border-radius:50%;background:#fff;opacity:0;transition:opacity .15s;box-shadow:0 1px 4px rgba(0,0,0,.5);pointer-events:none}#musemobilePlayerControls .spl-vol-bar:hover .spl-vol-handle,#musemobilePlayerControls .spl-vol-bar:active .spl-vol-handle{opacity:1}';
                    var t=document.head||document.documentElement;if(t)t.appendChild(sst);
                }
            ...
            if(document.readyState==='complete') initMuseMobilePlayer();
            else window.addEventListener('load',initMuseMobilePlayer);
