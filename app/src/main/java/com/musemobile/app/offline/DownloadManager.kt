package com.musemobile.app.offline
...
import com.musemobile.app.yt.CandidateScorer
import com.musemobile.app.yt.CandidateScorer.isAcceptableMatch
import com.musemobile.app.yt.YTPlayerUtils
...
object DownloadManager {
    ...
    private suspend fun runSingle(appContext: Context, track: TrackMeta) {
        if (OfflineStore.isTrackSaved(appContext, track.trackId)) {
            ...
            withContext(Dispatchers.Main) { onStatus?.invoke("Already saved to Music/MuseMobile") }
            return
        }
        ...
        try {
            ...
            when (result) {
                is TrackResult.Saved -> {
                    ...
                    onProgress?.invoke(100, "Saved to Music/MuseMobile")
                    withContext(Dispatchers.Main) { onStatus?.invoke("Saved to Music/MuseMobile") }
                }
    ...
    private suspend fun runCollection(appContext: Context, job: DownloadJob.Collection) {
        ...
        when {
            ...
            saved > 0 -> onProgress?.invoke(100, "$summary — Music/MuseMobile")
    ...
    private suspend fun downloadToFile(
        ...
        tmpFile.delete()
        if (uri != null) {
            Log.d(TAG, "downloadToFile: saved to Music/MuseMobile uri=$uri")
